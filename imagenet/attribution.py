...
def compute_attribution(image, method, clf, target, plot=False, ret_params=False, fixrange=None, p=0.0, ae=None, sigma=0, threshold=False):
    ...
    if "latentshift" in method:
        ...
        pred = torch.sigmoid(clf((image*p + xp*(1-p))))[:,target]
        ...
        def compute_shift(lam):
            if lam not in cache:
                ...
                pred1 = torch.sigmoid(clf((image*p + xpp*(1-p))))[:,target].detach().cpu().numpy()
                cache[lam] = xpp, pred1
                print(lam, pred1)
            return cache[lam]
...
#                     rbound = rbound + step
    ...
    if method == "guided":
        ...
        dimage = attr.attribute(image, target=target)
        dimage = dimage.detach().cpu().numpy()[0][0]
        dimage = clean(dimage)
        return dimage
...
def generate_video(image, model, target, ae, temp_path="/tmp/gifsplanation", method="latentshift", target_filename=None, border=True, note="", show=False, watermark=True, ffmpeg_path="ffmpeg", fixrange=None, params=None):
    
    if not params:
        params = compute_attribution(image, method, model, target, ret_params=True, ae=ae, fixrange=fixrange)
    #dimgs = params["dimgs"]
    dimgs = [h.transpose(0, 2,3,1) for h in params["dimgs"]]
    ...
    for idx, dimg in enumerate(towrite):
        ...
        #p = model(torch.from_numpy(dimg))[0,target].detach().cpu().numpy()

        if border:
            px = 1/plt.rcParams['figure.dpi']
            full_frame(dimg[0].shape[0]*px*2,dimg[0].shape[1]*px)
            plt.imshow(np.concatenate([img,dimg[0][0]], 1), interpolation='none', cmap='Greys_r')
            #plt.title("Pred of {}: {:.2f}".format(target, p),fontsize=25)
        else:
            px = 1/plt.rcParams['figure.dpi']
            full_frame(dimg[0].shape[0]*px,dimg[0].shape[1]*px)
            
            
            plt.imshow(dimg[0], interpolation='none', cmap='Greys_r')
        plt.axis('off')
...
def generate_attributions(sample, model, target, ae, temp_path, dmerge, plot_iou=False, methods = ["image", "grad", "guided", "integrated", "latentshift-max"], threshold=True):
    ...
    p = model(image)[:,target].detach().cpu()
    print(p)
